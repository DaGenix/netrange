...
use std::fs::File;
use std::str::FromStr;
use libnetrangemerge::{IpNetwork, NetworkInterest, merge_networks};
...
fn convert_to_ranges(aws_ip_ranges: AwsRanges) -> Result<Vec<NetworkInterest<IpNetwork>>, Error> {
    let lua = rlua::Lua::new();
    lua.context(|ctx| -> Result<(), Error> {
        let func = ctx.load("return region == 'us-east-1' and service == 'EC2'").into_function()?;
        ctx.globals().set("func", func)?;
        Ok(())
    })?;

    let ipv4_ranges = aws_ip_ranges
        .prefixes
        .into_iter()
        .map(|range| {
            ...
            let interesting = lua.context(|ctx| -> Result<bool, Error> {
                ctx.globals().set("region", range.region)?;
                ctx.globals().set("service", range.service)?;
                ctx.globals().set("network_border_group", range.network_border_group)?;
                let func: rlua::Function<'_> = ctx.globals().get("func")?;
                Ok(func.call(())?)
            })?;

            Ok(NetworkInterest::new(
                network,
                interesting,
            ))
        });
    let ipv6_ranges = aws_ip_ranges
        .ipv6_prefixes
        .into_iter()
        .map(|range| {
...
fn main() -> Result<(), Error> {
    ...
    let mut all_networks = convert_to_ranges(aws_ranges)?;

    // AWS owns some IP ranges that we know about - so, we can add that in
